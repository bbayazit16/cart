...
impl<T> CartArray<T> {
    pub fn new(initial_capacity: usize) -> Self {
        let elements = unsafe {
            std::alloc::alloc(
                std::alloc::Layout::array::<T>(initial_capacity)
        ...
        Self {
            ref_count: 1,
            size: 0,
            capacity: initial_capacity as i32,
            elements,
        }
    }
    ...
    pub fn push(&mut self, value: T) {
        if self.size == self.capacity {
            self.resize_to((self.capacity * 2) as usize);
        }
        unsafe {
            *self.elements.add(self.size as usize) = value;
        }
        self.size += 1;
    }
    ...
    where
        T: Copy,
    {
        let total_elements = self.size as usize + values.len();
        if total_elements > self.capacity as usize {
            self.resize_to(total_elements);
        }
    ...
    fn resize_to(&mut self, new_capacity: usize) {
        ...
        let new_elements = unsafe {
            let new_ptr = std::alloc::alloc(
                std::alloc::Layout::array::<T>(self.capacity as usize)
                    .expect("Failed to allocate memory"),
            ) as *mut T;
            std::ptr::copy_nonoverlapping(self.elements, new_ptr, self.size as usize);
            std::alloc::dealloc(
