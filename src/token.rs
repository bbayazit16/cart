    ...
    ( $( $name:ident $( ( $( $type:ty ),* ) )? ),* $(,)? ) => {
        ...
        impl Token {
            ...
            pub fn get_line_position(&self) -> usize {
                match self {
                    $(
                        Token::$name(_, _, line_pos, ..) => *line_pos,
                    )*
                }
            }
        }
...
define_tokens!(
    ...
    RightAngle(usize, usize, usize),         // >
    GreaterEqual(usize, usize, usize),       // >=
    LeftAngle(usize, usize, usize),          // <
    ...
    // Keywords
    Enum(usize, usize, usize),       // enum
    Error(usize, usize, usize),      // error
