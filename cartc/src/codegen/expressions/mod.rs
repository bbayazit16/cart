    ...
    pub(super) fn generate_expression(&mut self, expr: &Expression) -> Option<Value<'ctx>> {
        match expr {
            ...
            Expression::Unary {
                ...
                ref op,
                ref ty,
            } => Some(self.generate_unary(expr, op, ty)),
            Expression::Binary {
                ref left,
                ref left_type,
                ref op,
            ...
            Type::Bool => match op {
                BinaryOp::And => {
                    ...
                    let bool_result = self
                        .builder
                        .build_int_truncate(and, self.context.bool_type(), "and_bool_to_bool")
                        .unwrap();
                ...
                BinaryOp::Or => {
                    ...
                    let bool_result = self
                        .builder
                        .build_int_truncate(or, self.context.bool_type(), "or_bool_to_bool")
                        .unwrap();
    ...
    fn generate_unary(&mut self, expr: &Expression, op: &UnaryOp, ty: &Type) -> Value<'ctx> {
        ...
        match ty {
            ...
            Type::Bool => {
                ...
                let res = match op {
                    UnaryOp::Not => self.builder.build_not(int_value, "not").unwrap(),
